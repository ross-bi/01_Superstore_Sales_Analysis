...
LOAD DATA LOCAL INFILE 'C:\\Program Files\\MySQL\\MySQL Server 8.0\\Data\\01_Superstore_Project\\data\\superstore_clean.csv'
...
(order_id, order_date, ship_date, ship_mode, customer_name, segment,
 ...
 order_priority, @dummy);  -- 上面最後的 @dummy 用來丟掉多餘的欄位，不匯入到表格

SHOW WARNINGS;
SELECT COUNT(*) AS 匯入筆數 FROM staging_sales;  -- 顯示匯入過程的警告訊息 (例如重複主鍵、資料截斷等)