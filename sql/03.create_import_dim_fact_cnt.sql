...
-- 移除 raw_product_id UNIQUE，改用複合主鍵
CREATE TABLE IF NOT EXISTS dim_product (
    product_id INT AUTO_INCREMENT PRIMARY KEY,
    raw_product_id VARCHAR(20) NOT NULL,
    product_name VARCHAR(255) NOT NULL,
    sub_category_id INT,
    -- 複合唯一鍵：同一個 raw_product_id + product_name 才唯一
    UNIQUE KEY uk_product_combo (raw_product_id, product_name),
...
-- INSERT IGNORE 會自動處理重複，允許多個 product_name
...
FROM staging_sales s 
JOIN dim_sub_category sc ON s.sub_category = sc.sub_category_name;
...
SELECT 
    '產品衝突總結' as 檢查項目,
    COUNT(DISTINCT raw_product_id) as '衝突product_id數量',
    SUM(name_count - 1) as '總衝突數量',
    COUNT(*) as '總有衝突的product_id'
FROM (
    SELECT raw_product_id, COUNT(DISTINCT product_name) as name_count
    FROM dim_product 
